...
## How This Preset Differs

The `node-mysql` preset is an overlay on the default `cloudflare-d1` app. Pages,
layouts, components, Astro Actions, middleware, and app-owned auth policy stay
the same. The preset replaces the runtime and database boundaries, along with
the operational files and tests that exercise them.

| Concern | Default `cloudflare-d1` | `node-mysql` |
| --- | --- | --- |
| Runtime | Cloudflare Workers adapter and request bindings | Standalone Node.js adapter and `process.env` |
| Database | D1/SQLite through a Worker binding | MySQL through a `mysql2` connection pool |
| Local configuration | `.dev.vars` and `wrangler.jsonc` | `.env` created from `.env.example` |
| Migrations | `db:migrate:local` and `db:migrate:remote` through Wrangler | `db:migrate` through Drizzle Kit |
| Email providers | Console, Resend, Mailgun, and Cloudflare Email | Console, Resend, and Mailgun |
| Production | `wrangler deploy` to Cloudflare | `npm run start` behind a process manager and reverse proxy |

In the generated source, the main replacements are `astro.config.mjs`,
`drizzle.config.ts`, `src/db.ts`, `src/runtime.ts`, `src/env.d.ts`,
`src/config/schema.ts`, the migration tree, and their runtime-specific tests.
The rest of the documentation still applies unless this page gives a different
configuration, migration, email, or deployment instruction.
...
In an interactive terminal, the installer creates `.env` with a fresh Better
Auth secret and can run `npm install` for you. It then groups the MySQL host,
port, database, user, and masked password questions into one connection step,
using `.env.example` values as the defaults. After saving those five settings,
it can run the migration and launch administrator creation. If `.env` already
exists, its secrets and unrelated settings are preserved; only the five MySQL
values you explicitly confirm are updated.
...
If you skip the guided connection step or run non-interactively, set the local
values in `.env`:
...
If the installer did not create `.env`, generate a Better Auth secret with
Node.js:
