...
These components are included by default: [Button](https://ui.bejamas.com/components/button), [Field](https://ui.bejamas.com/components/field), [Input](https://ui.bejamas.com/components/input), [Label](https://ui.bejamas.com/components/label), [Separator](https://ui.bejamas.com/components/separator)
...
Most shadcn theme generators (such as [Shadcn Themer](https://shadcnthemer.com/), [tweakcn](https://tweakcn.com/editor/theme), and [clonecn](https://github.com/hunvreus/clonecn)) work with the Bejamas color variables. Use [Bejamas Create](https://ui.bejamas.com/create) when you need a complete preset instead of colors only.

### Create a theme

The fastest method is to create a preset with [Bejamas Create](https://ui.bejamas.com/create). Commit your changes, then apply only its theme values:

```bash
npx bejamas apply <preset> --only theme
```

You can also copy a generator's CSS values into `src/styles/global.css`. Define light values in `:root` and dark values in `.dark`:
